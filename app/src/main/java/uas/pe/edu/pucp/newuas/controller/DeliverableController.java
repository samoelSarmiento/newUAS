...
/*
    ...
    public void registerObs (final Context context, final InvDocument invDocument){
        RestCon restCon = RetrofitHelper.apiConnector.create(RestCon.class);
        //Toast.makeText(context,"llegue", Toast.LENGTH_SHORT).show();
        Map<String, String> token = new HashMap<>();
        token.put("token", Configuration.LOGIN_USER.getToken());
        Call<StringResponse> call = restCon.registerObs(invDocument.getId(),token,invDocument);
